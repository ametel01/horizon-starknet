...
/// Standardized Yield with Rewards (SYWithRewards) Token
///
/// Extends the base SY functionality with integrated reward distribution.
/// Wraps yield-bearing tokens into a standardized interface while tracking
/// reward token distributions to all SY holders proportionally.
///
/// Architecture (Composition via Components):
/// ```
/// ┌──────────────────────────────────────────────────────────────────────────┐
...
/// ┌───────────────┐
/// ┌──────────────┐  │
/// │  │ ERC20Comp  │  │ SYComponent  │  │ RewardManager │  │ Pausable/    │  │
/// │  │            │  │              │  │ Component     │  │ Reentrancy   │  │
/// │  │ - balances │◄─┤ - deposit()  │  │               │  │              │
/// │
/// │  │ - supply   │  │ - redeem()   │  │ - claim()     │  │ - paused     │  │
/// │  │ - transfer │  │ - rate()     │  │ - indices     │  │              │  │
/// │  └─────▲──────┘
/// └──────────────┘
/// └───────▲───────┘
/// └──────────────┘  │
/// │        │                                   │                             │
/// │        │  ERC20HooksTrait                 │  RewardHooksTrait           │
/// │        │  before_update() ────────────────┼──►
/// update_rewards_for_two() │
/// │        │                                   │                             │
/// │
/// └───────────────────────────────────┘
/// │
...
/// │  - ERC20: before_update() → update rewards BEFORE balance changes        │
/// │  - SY: after_deposit/redeem() → update user rewards (already minted)     │
/// │  - Reward: user_sy_balance() → reads from ERC20 component                │
/// └──────────────────────────────────────────────────────────────────────────┘
/// ```
///
...
pub mod SYWithRewards {
    ...
    use horizon::interfaces::i_sy_with_rewards::{ISYWithRewards, ISYWithRewardsAdmin};
    ...
    // Only use internal impls - do NOT embed MixinImpls to avoid duplicate entry points
    ...
    // Reward component internal impl with hooks
    ...
    /// Implement SYHooksTrait - bridge SYComponent to ERC20, rewards, and security components
    ///
    /// The composition pattern: SY component calls these hooks and we delegate
    /// to the appropriate components (ERC20 for minting, rewards for tracking, etc.)
    impl SYHooksImpl of SYComponent::SYHooksTrait<ContractState> {
        ...
        /// After deposit: update receiver's rewards and end reentrancy guard
        /// Note: The mint already triggered ERC20 hooks which updated rewards,
        /// but we call update_user_rewards again to ensure their index is set
        /// to current global (important for first-time depositors)
        fn after_deposit(ref self: ContractState, receiver: ContractAddress, sy_minted: u256) {
            // Update rewards for the receiver after their balance changed
            self.rewards.update_user_rewards(receiver);
        ...
        /// After redeem: update user's rewards and end reentrancy guard
        fn after_redeem(ref self: ContractState, receiver: ContractAddress, amount_redeemed: u256) {
            // Receiver is the one who gets underlying, but the burner's rewards
            // are already updated via ERC20 burn hooks
            self.reentrancy_guard.end();
        }
    }

    /// Implement RewardHooksTrait - bridge RewardManager to ERC20 for balance info
    ///
    /// The RewardManager needs balance information to calculate rewards.
    ...
    /// @param reward_tokens Reward tokens to track (at least one required)
    #[constructor]
    fn constructor(
        ...
        // Initialize RewardManager component
    ...
    impl SYWithRewardsImpl of ISYWithRewards<ContractState> {
        ...
        // ============ SY Core Methods (delegating to SYComponent) ============
    ...
    #[abi(embed_v0)]
    impl SYWithRewardsAdminImpl of ISYWithRewardsAdmin<ContractState> {
