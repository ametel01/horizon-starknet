...
fn test_twap_increases_over_time() {
    ...
    // Query TWAP for a window 6 months after deployment
    // This simulates checking price 6 months after deployment
    let start_time = deployment_ts + 15768000; // 6 months after deployment
    let (twap, decimals) = pragma.calculate_twap(data_type, aggregation_mode, 3600, start_time);

    // Expected: base_price * (1 + 8% * 0.5) = 50000000 * 1.04 = 52000000
    // The midpoint is at start_time + 1800, which is ~6 months after deployment
    // Yield accrual = base * rate * time / (10000 * year)
    // = 50000000 * 800 * 15769800 / (10000 * 31536000)
    // = 50000000 * 800 * 0.5 / 10000
    // = 2000000
...
fn test_twap_one_year_full_yield() {
    ...
    // Query TWAP for a window 1 year after deployment
    let start_time = deployment_ts + 31536000; // 1 year after deployment
    ...
    // The midpoint is at start_time + 1800, which is ~1 year after deployment
...
fn test_wsteth_yield_accrual() {
    ...
    // Query TWAP 1 year after deployment
