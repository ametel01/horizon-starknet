/// Tests for dual-token LP operations (add_liquidity_dual_token_and_pt,
/// remove_liquidity_dual_token_and_pt)
///
/// These tests focus on:
/// - Basic functionality of dual-token liquidity operations through aggregator
/// - User receives/provides both token + PT in these operations
...
// Helper: Setup user with PT tokens for dual liquidity add
fn setup_user_with_pt(
    underlying: IMockERC20Dispatcher,
    sy: ISYDispatcher,
    yt: IYTDispatcher,
    user: ContractAddress,
    ...
    // Mint SY first
    setup_user_with_sy(underlying, sy, user, pt_amount);

    // Mint PT+YT from SY
    start_cheat_caller_address(sy.contract_address, user);
    sy.transfer(yt.contract_address, pt_amount);
    stop_cheat_caller_address(sy.contract_address);
...
// ============ add_liquidity_dual_token_and_pt Tests ============

#[test]
fn test_add_liquidity_dual_token_and_pt_basic() {
    ...
    setup_user_with_pt(underlying, sy, yt, user, pt_amount);

    // Approve router for external token and PT
    ...
    // Execute add_liquidity_dual_token_and_pt
    start_cheat_caller_address(router.contract_address, user);
    let lp_out = router
        .add_liquidity_dual_token_and_pt(
            market.contract_address, user, input, pt_amount, 0, DEFAULT_DEADLINE,
        );
...
#[test]
fn test_add_liquidity_dual_token_and_pt_to_receiver() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let sender = user1();
    let receiver = user2();
    ...
    let pt_amount = 10 * WAD;
    setup_user_with_pt(underlying, sy, yt, sender, pt_amount);
    ...
    let lp_out = router
        .add_liquidity_dual_token_and_pt(
            market.contract_address, receiver, input, pt_amount, 0, DEFAULT_DEADLINE,
        );
    stop_cheat_caller_address(router.contract_address);
...
fn test_add_liquidity_dual_token_and_pt_slippage_protection() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();
    ...
    router
        .add_liquidity_dual_token_and_pt(
            market.contract_address, user, input, pt_amount, token_amount * 1000, DEFAULT_DEADLINE,
        );
}

#[test]
#[should_panic(expected: ('HZN: deadline exceeded',))]
fn test_add_liquidity_dual_token_and_pt_deadline() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();
    ...
    router
        .add_liquidity_dual_token_and_pt(
            market.contract_address, user, input, pt_amount, 0, CURRENT_TIME + 500,
        ); // deadline passed
}

// ============ remove_liquidity_dual_token_and_pt Tests ============

#[test]
fn test_remove_liquidity_dual_token_and_pt_basic() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();
    ...
    // Now remove liquidity dual token and pt
    let lp_token = IERC20Dispatcher { contract_address: market.contract_address };
    ...
    // Execute remove_liquidity_dual_token_and_pt
    start_cheat_caller_address(router.contract_address, user);
    let (token_out, pt_out) = router
        .remove_liquidity_dual_token_and_pt(
            market.contract_address, user, lp_out, output, 0, DEFAULT_DEADLINE,
        );
    stop_cheat_caller_address(router.contract_address);
...
#[test]
fn test_remove_liquidity_dual_token_and_pt_to_receiver() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let sender = user1();
    let receiver = user2();
    ...
    let lp_out = router
        .add_liquidity_dual_token_and_pt(
            market.contract_address, sender, input, pt_amount, 0, DEFAULT_DEADLINE,
        );
    stop_cheat_caller_address(router.contract_address);
    ...
    let (token_out, pt_out) = router
        .remove_liquidity_dual_token_and_pt(
            market.contract_address, receiver, lp_out, output, 0, DEFAULT_DEADLINE,
        );
    stop_cheat_caller_address(router.contract_address);
...
#[test]
#[should_panic(expected: ('HZN: slippage exceeded',))]
fn test_remove_liquidity_dual_token_and_pt_pt_slippage_protection() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();
    ...
    // Request unrealistic min_pt_out (should fail)
    let output = TokenOutput {
        token: external_token.contract_address,
        min_amount: 0, // token slippage is fine
        swap_data: SwapData { aggregator: aggregator.contract_address, calldata: array![].span() },
    };
    ...
    router
        .remove_liquidity_dual_token_and_pt(
            ...
            lp_out,
            output,
            pt_amount * 1000, // unrealistic PT slippage
            DEFAULT_DEADLINE,
        );
}
...
fn test_remove_liquidity_dual_token_and_pt_token_slippage_protection() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();
    ...
    // Request unrealistic token min_amount (should fail via aggregator)
    let output = TokenOutput {
        token: external_token.contract_address,
        min_amount: token_amount * 1000, // unrealistic token slippage
        swap_data: SwapData { aggregator: aggregator.contract_address, calldata: array![].span() },
    };
...
#[test]
#[should_panic(expected: ('HZN: deadline exceeded',))]
fn test_remove_liquidity_dual_token_and_pt_deadline() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();
    ...
    router
        .remove_liquidity_dual_token_and_pt(
            market.contract_address, user, lp_out, output, 0, CURRENT_TIME + 500,
        ); // deadline passed
}
...
#[test]
fn test_add_liquidity_dual_token_and_pt_returns_dust() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();

    // Initialize pool with imbalanced ratio - more SY than PT needed
    // This means some PT might not be used
    initialize_pool(underlying, sy, yt, pt, market, user2(), 200 * WAD, 50 * WAD);
    ...
    // Mint external tokens to user - significantly more than we expect SY side to need
    ...
    // The function should return dust (SY or PT not used) to the caller
    // Given the imbalanced pool, one side should have dust returned
    // Either sy_before < sy.balance_of(user) or pt_before < pt.balance_of(user)
    let sy_dust = sy.balance_of(user) - sy_before;
    let pt_dust = pt.balance_of(user) - (pt_before - pt_amount);

    // At least some dust should be returned (either SY or unused PT)
    // This test validates the dust return mechanism works
    assert(sy_dust > 0 || pt_dust > 0, 'No dust returned');
}

// ============ Round-trip Tests ============

#[test]
fn test_dual_token_and_pt_round_trip() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();
    ...
    let initial_token_balance = external_token.balance_of(user);
    let initial_pt_balance = pt.balance_of(user);
    ...
    let final_token_balance = external_token.balance_of(user);
    let final_pt_balance = pt.balance_of(user);

    assert(token_out > 0, 'No token out');
    assert(pt_out > 0, 'No PT out');

    // Due to fees, we expect some loss but reasonable recovery
    // At least 70% recovery after all fees
    assert(final_token_balance > initial_token_balance * 70 / 100, 'Too much token loss');
    assert(final_pt_balance > initial_pt_balance * 70 / 100, 'Too much PT loss');
}
...
fn test_add_liquidity_dual_token_and_pt_zero_market() {
    ...
    start_cheat_caller_address(router.contract_address, user);
    router.add_liquidity_dual_token_and_pt(zero_address, user, input, WAD, 0, DEFAULT_DEADLINE);
}

#[test]
#[should_panic(expected: ('HZN: zero address',))]
fn test_add_liquidity_dual_token_and_pt_zero_receiver() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();
    let zero_address: ContractAddress = 0.try_into().unwrap();
    ...
    router
        .add_liquidity_dual_token_and_pt(
            market.contract_address, zero_address, input, WAD, 0, DEFAULT_DEADLINE,
        );
}

#[test]
#[should_panic(expected: ('HZN: zero amount',))]
fn test_add_liquidity_dual_token_and_pt_zero_token_amount() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();
    ...
    router
        .add_liquidity_dual_token_and_pt(
            market.contract_address, user, input, WAD, 0, DEFAULT_DEADLINE,
        );
}

#[test]
#[should_panic(expected: ('HZN: zero amount',))]
fn test_add_liquidity_dual_token_and_pt_zero_pt_amount() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();
    ...
    router
        .add_liquidity_dual_token_and_pt(
            market.contract_address, user, input, 0, // zero PT amount
            0, DEFAULT_DEADLINE,
        );
}
...
fn test_add_liquidity_dual_token_and_pt_zero_aggregator() {
...
#[test]
#[should_panic(expected: ('HZN: zero amount',))]
fn test_remove_liquidity_dual_token_and_pt_zero_lp() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();
    ...
    router
        .remove_liquidity_dual_token_and_pt(
            market.contract_address, user, 0, output, 0, DEFAULT_DEADLINE,
        ); // zero LP
}
...
#[test]
fn test_add_dual_token_and_pt_with_favorable_exchange_rate() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();
    ...
    // With 2:1 rate, we should get LP
    assert(lp_out > 0, 'No LP received');
}

#[test]
fn test_add_dual_token_and_pt_with_unfavorable_exchange_rate() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();
...
#[test]
fn test_dual_token_and_pt_imbalanced_pool_more_sy() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();
...
#[test]
fn test_dual_token_and_pt_imbalanced_pool_more_pt() {
    let (underlying, _, sy, yt, pt, market, router, aggregator, external_token) =
        deploy_test_stack();
    let user = user1();
