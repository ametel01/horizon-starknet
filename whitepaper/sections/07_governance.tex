...
This section specifies the governance structure, upgrade capabilities, and admin powers. All protocol contracts are upgradeable; users must evaluate admin trust before depositing.
...
PT & Yes & Redemption logic, mint authority validation & Critical \\
YT & Yes & Yield distribution, interest index accounting & Critical \\
Market & Yes & AMM curve, swap execution, fee calculation & Critical \\
...
All admin functions require \texttt{DEFAULT\_ADMIN\_ROLE}. This section enumerates the exact on-chain powers.
...
\texttt{set\_default\_fee\_rate} & Configure default swap fee for new markets & Medium \\
\texttt{set\_default\_scalar\_root} & Configure default price sensitivity & Medium \\
\texttt{set\_default\_initial\_anchor} & Configure default implied rate anchor & Medium \\
\bottomrule
\end{tabularx}
\end{center}
...
\textbf{Function} & \textbf{Effect} & \textbf{Risk} \\
\midrule
\texttt{set\_staleness\_threshold} & Configure maximum age for oracle data & Medium \\
\texttt{set\_twap\_duration} & Configure TWAP averaging window & Medium \\
\texttt{set\_feed\_id} & Change Pragma feed identifier & High \\
\bottomrule
\end{tabularx}
\end{center}

\paragraph{Token Contracts (SY, PT, YT).}
...
\paragraph{Market.}
Market contracts expose only \texttt{upgrade}. AMM parameters (fee rate, scalar root, anchor) are set at deployment and cannot be modified post-deployment.
...
\texttt{PAUSER\_ROLE} & \texttt{pause()}, \texttt{unpause()} on Router & Owner (initially) \\
\texttt{OPERATOR\_ROLE} & Reserved for future operational functions & Not assigned \\
\bottomrule
\end{tabularx}
\end{center}
...
The pause mechanism affects only the Router contract. Its purpose is to stop new protocol interactions while preserving user exit paths.
