...
SYWithRewards & SY plus reward tracking & Medium & Fully backed by underlying \\
...
RouterStatic & Read-only quotes & Low & No state changes \\
PyLpOracle & PT/YT/LP TWAP helper & Medium & Uses Market oracle state \\
\bottomrule
\end{tabular}
\end{center}
...
\item After expiry, YT stops accruing yield; transfers remain allowed.
\end{itemize}
...
\item If reward tokens are configured, YT exposes reward helpers (e.g., \texttt{claim\_rewards}, \texttt{get\_reward\_tokens}, \texttt{redeem\_due\_interest\_and\_rewards}).
\end{itemize}
...
\item Swap fees can include a reserve fee to the treasury; optional LP reward tokens may be tracked.
\end{itemize}
