...
function processYieldData(data: YieldResponse): ProcessedYieldData {
	return {
		...
		summaryByPosition: data.summaryByPosition.map((s) => ({
			...
			totalGrossYield: s.totalGrossYield
				? BigInt(s.totalGrossYield)
				: BigInt(s.totalClaimed),
		})),
...
export interface UseUserYieldReturn {
	data: Accessor<ProcessedYieldData | undefined>;
...
export function useUserYield(
	...
	const query = createQuery(() => {
		...
		return {
			queryKey: ["user-yield", currentAddress, days],
			queryFn: async () => {
				if (!currentAddress) throw new Error("No address");
				const data = await fetchYieldData(currentAddress, days);
...
export interface UseYieldClaimPreviewReturn {
	data: Accessor<YieldClaimPreview | null | undefined>;
	isLoading: Accessor<boolean>;
	isError: Accessor<boolean>;
	error: Accessor<Error | null>;
	address: Accessor<string | null>;
 ...
 *
 * @param ytAddress - Accessor returning the YT contract address
...
export function useYieldClaimPreview(
	ytAddress: Accessor<string | undefined>,
): UseYieldClaimPreviewReturn {
	const { provider } = useStarknet();
	const { address } = useAccount();

	const query = createQuery(() => {
		const currentYtAddress = ytAddress();
		const currentAddress = address();

		return {
			queryKey: ["yield-claim-preview", currentYtAddress, currentAddress],
			queryFn: async (): Promise<YieldClaimPreview | null> => {
				if (currentYtAddress === undefined || !currentAddress) {
					return null;
				}

				const yt = getYTContract(currentYtAddress, provider);

				// Fetch pending interest and fee rate in parallel
				const [rawPendingInterest, rawFeeRate] = await Promise.all([
					yt.get_user_interest(currentAddress),
			...
			enabled: currentYtAddress !== undefined && !!currentAddress,
