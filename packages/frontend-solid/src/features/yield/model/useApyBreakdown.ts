 ...
 */
function getEstimatedUnderlyingApy(
	...
	const query = createQuery(() => {
		...
		return {
			...
			queryFn: async (): Promise<SyRateData> => {
				...
				const currentRate = toBigInt(
					exchangeRate as bigint | { low: bigint; high: bigint },
				);
				...
				const previousRate =
					(currentRate * WAD_BIGINT) / (WAD_BIGINT + estimatedDailyGrowth);
...
export function useApyBreakdown(
	...
	const yieldTokenSymbol = createMemo(
		() => market()?.metadata?.yieldTokenSymbol,
	);
	...
	const breakdown = createMemo((): MarketApyBreakdown | null => {
		...
		const estimatedDailyVolume =
			(marketData.state.syReserve + marketData.state.ptReserve) / 200n;
