...
export const portfolioKeys = {
	...
	user: (address: string | null) =>
		[...portfolioKeys.all, "user", address] as const,
	summary: (address: string | null, marketAddresses: string[]) =>
		[
			...portfolioKeys.user(address),
			"summary",
...
export function usePortfolio(
	...
	const summary = createMemo((): PortfolioSummary => {
		...
		const totalSyBalance = filteredPositions.reduce(
			(sum, p) => sum + p.syBalance,
			BigInt(0),
		);
		const totalPtBalance = filteredPositions.reduce(
			(sum, p) => sum + p.ptBalance,
			BigInt(0),
		);
		const totalYtBalance = filteredPositions.reduce(
			(sum, p) => sum + p.ytBalance,
			BigInt(0),
		);
		const totalLpBalance = filteredPositions.reduce(
			(sum, p) => sum + p.lpBalance,
			BigInt(0),
		);
