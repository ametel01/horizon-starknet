...
export async function fetchMarketPosition(
	...
	const syBalance = toBigInt(
		syBalanceResult as bigint | { low: bigint; high: bigint },
	);
	const ptBalance = toBigInt(
		ptBalanceResult as bigint | { low: bigint; high: bigint },
	);
	const ytBalance = toBigInt(
		ytBalanceResult as bigint | { low: bigint; high: bigint },
	);
	const lpBalance = toBigInt(
		lpBalanceResult as bigint | { low: bigint; high: bigint },
	);
	const claimableYield = toBigInt(
		claimableYieldResult as bigint | { low: bigint; high: bigint },
	);
...
export const positionKeys = {
	...
	user: (address: string | null) =>
		[...positionKeys.all, "user", address] as const,
	markets: (address: string | null, marketAddresses: string[]) =>
		[
			...positionKeys.user(address),
			"markets",
...
export function usePositions(
		...
		queryFn: async (): Promise<PortfolioData> => {
			...
			const positions = await Promise.all(
				marketList.map((market) =>
					fetchMarketPosition(market, userAddress, provider),
				),
			);
	...
	const totalClaimableYield = createMemo(
		() => query.data?.totalClaimableYield ?? BigInt(0),
	);
	const hasClaimableYield = createMemo(
		() => query.data?.hasClaimableYield ?? false,
	);
	const hasRedeemablePositions = createMemo(
		() => query.data?.hasRedeemablePositions ?? false,
	);
...
export function useHasPosition(
	market: Accessor<MarketData | undefined>,
): Accessor<boolean> {
	const { provider } = useStarknet();
	const { address } = useAccount();
	...
	const query = createQuery(() => ({
		...
		queryFn: async (): Promise<boolean> => {
			...
			const position = await fetchMarketPosition(
				marketData,
				userAddress,
				provider,
			);
