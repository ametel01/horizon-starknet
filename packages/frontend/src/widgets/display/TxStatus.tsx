...
interface ErrorDisplayProps {
  ...
  showSlippageAction: boolean;
  showDeadlineHint: boolean;
  onAdjustSlippage?: (() => void) | undefined;
}
...
/**
 * Get card styling class based on status
 */
function getStatusCardClass(status: TxStatusType): string {
  switch (status) {
    case 'signing':
      return 'border-chart-1/20 bg-chart-1/10';
    case 'pending':
 ...
 */
function getStatusTextClass(status: TxStatusType): string {
  switch (status) {
    case 'signing':
      return 'text-chart-1';
    case 'pending':
...
export function TxStatus({
  ...
  return (
    <Card
      size="sm"
      className={cn(getStatusCardClass(status), className)}
      role="status"
      aria-live="polite"
      aria-atomic="true"
    >
      <CardContent className="p-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className={cn('font-medium', getStatusTextClass(status))}>{statusText}</p>
            {txHash ? (
            ...
            {errorMessage ? (
              <ErrorDisplay
                ...
                helpText={helpText}
                showSlippageAction={showSlippageAction}
                showDeadlineHint={showDeadlineHint}
                onAdjustSlippage={onAdjustSlippage}
              />
            ) : null}
